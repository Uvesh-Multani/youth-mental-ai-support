...
import { BookOpen, Brain, HeartHandshake, GraduationCap, PhoneCall, Headphones, Search, ArrowRight } from "lucide-react";
...
export default function ResourcesPage() {
  return (
    <div className="min-h-screen">
      <header className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-5xl px-6 py-16 md:py-24 text-center">
          ...
          <h1 className="mt-4 text-3xl md:text-5xl font-extrabold tracking-tight">
            Wellness Resources, <span className="bg-gradient-to-r from-violet-600 to-sky-500 bg-clip-text text-transparent">Supercharged</span> for Indian Youth
          </h1>
      ...
      <main id="resources" className="mx-auto max-w-5xl px-6 py-10 grid gap-8">
        ...
        <section className="rounded-xl border bg-card p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold">Helplines & Services (India)</h2>
          </div>
          <ul className="mt-3 space-y-2 text-muted-foreground">
            <li>
              KIRAN (24x7 National Helpline): <a className="underline" href="tel:18005990019">1800‑599‑0019</a>
            </li>
            <li>
              iCall (TISS): <a className="underline" href="tel:9152987821">9152987821</a> | <a className="underline" href="https://icallhelpline.org" target="_blank" rel="noreferrer">icallhelpline.org</a>
            </li>
            <li>
              AASRA (Suicide Prevention): <a className="underline" href="tel:+919820466726">+91‑9820466726</a>
            </li>
          </ul>
          <p className="mt-3 text-sm text-muted-foreground">If you are in immediate danger, call 112.</p>
        </section>

        <div className="flex flex-wrap gap-3">
          <Link href="/" className="rounded-md border px-4 py-2 hover:bg-accent">Back home</Link>
          <Link href="/journal" className="rounded-md border px-4 py-2 hover:bg-accent">Open mood journal</Link>
          <Link href="/resources" className="inline-flex items-center gap-2 rounded-md bg-foreground px-4 py-2 text-background hover:opacity-90">
            <BookOpen className="h-4 w-4" /> More to read
          </Link>
        </div>
      </main>

      {/* Footer */}
      <footer className="mt-6 border-t bg-gradient-to-b from-background to-secondary/40">
        <div className="mx-auto max-w-6xl px-6 py-10">
          <div className="flex flex-col items-center justify-between gap-6 sm:flex-row">
            <div className="text-center sm:text-left">
              <div className="text-xl font-bold">
                <span className="bg-gradient-to-r from-violet-600 to-sky-500 bg-clip-text text-transparent">Bloom</span>
                <span className="ml-1"> Wellness</span>
              </div>
              <p className="mt-1 text-sm text-muted-foreground">Confidential, India‑aware support for youth.</p>
            </div>
            <nav className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
              <Link href="/resources" className="hover:text-foreground">Resources</Link>
              <Link href="/journal" className="hover:text-foreground">Journal</Link>
              <Link href="/crisis" className="hover:text-foreground">Crisis Support</Link>
              <Link href="/login" className="hover:text-foreground">Sign in</Link>
            </nav>
          </div>
          <div className="mt-6 text-center text-xs text-muted-foreground">© {new Date().getFullYear()} Bloom Wellness. All rights reserved.</div>
        </div>
      </footer>
    </div>
  );
